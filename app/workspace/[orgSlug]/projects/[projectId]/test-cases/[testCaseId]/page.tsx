    ...
    <div className="mx-auto max-w-5xl">
      ...
      {testCase.status === "DRAFT" && detail.canUpdate ? (
        <form action={updateAction} className="mt-8 space-y-5 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
          ...
          <label className="block text-sm font-medium">Title<input name="title" required defaultValue={testCase.title} className="mt-2 w-full rounded-lg border border-slate-300 px-3 py-2.5" /></label>
          <label className="block text-sm font-medium">Objective<textarea name="objective" rows={4} defaultValue={testCase.objective} className="mt-2 w-full rounded-lg border border-slate-300 px-3 py-2.5" /></label>
          <label className="block text-sm font-medium">Preconditions<textarea name="preconditions" rows={3} defaultValue={testCase.preconditions} className="mt-2 w-full rounded-lg border border-slate-300 px-3 py-2.5" /></label>
          <div className="grid gap-5 sm:grid-cols-2"><label className="block text-sm font-medium">Steps — one per line<textarea name="steps" rows={8} defaultValue={steps.join("\n")} className="mt-2 w-full rounded-lg border border-slate-300 px-3 py-2.5" /></label><label className="block text-sm font-medium">Expected results — one per line<textarea name="expectedResults" rows={8} defaultValue={expectedResults.join("\n")} className="mt-2 w-full rounded-lg border border-slate-300 px-3 py-2.5" /></label></div>
          <div className="grid gap-5 sm:grid-cols-2"><label className="block text-sm font-medium">Type<select name="type" defaultValue={testCase.type} className="mt-2 w-full rounded-lg border border-slate-300 px-3 py-2.5"><option value="FUNCTIONAL">Functional</option><option value="END_TO_END">End to end</option><option value="API">API</option><option value="INTEGRATION">Integration</option><option value="REGRESSION">Regression</option></select></label><label className="block text-sm font-medium">Priority<select name="priority" defaultValue={testCase.priority} className="mt-2 w-full rounded-lg border border-slate-300 px-3 py-2.5"><option>LOW</option><option>MEDIUM</option><option>HIGH</option><option>CRITICAL</option></select></label></div>
          <label className="block text-sm font-medium">Tags<input name="tags" defaultValue={testCase.tags.join(", ")} className="mt-2 w-full rounded-lg border border-slate-300 px-3 py-2.5" /></label>
