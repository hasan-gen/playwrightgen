    ...
    <div className="mx-auto max-w-2xl">
      ...
      <form action={createProjectAction} className="mt-8 space-y-5 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        <label className="block text-sm font-medium">
          Name
          <input name="name" required maxLength={200} className="mt-2 w-full rounded-lg border border-slate-300 px-3 py-2.5 outline-none focus:border-sky-500" />
        </label>
        <label className="block text-sm font-medium">
          Slug
          <input name="slug" required maxLength={100} pattern="[a-z0-9]+(?:-[a-z0-9]+)*" placeholder="release-confidence" className="mt-2 w-full rounded-lg border border-slate-300 px-3 py-2.5 outline-none focus:border-sky-500" />
        </label>
        <label className="block text-sm font-medium">
          Description
          <textarea name="description" rows={5} maxLength={10000} className="mt-2 w-full rounded-lg border border-slate-300 px-3 py-2.5 outline-none focus:border-sky-500" />
        </label>
