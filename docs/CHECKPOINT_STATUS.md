...
| V1.8 Repository, CI, and isolated execution | In progress | Least-privilege GitHub App and isolated-runner decisions recorded; composite-tenant installation/connection/import schema; immutable repository inventory; signed idempotent lifecycle handling; and an Owner/Admin setup flow with expiring state, PKCE, GitHub user-installation verification, strict App permission verification, live repository discovery, and provider-verified project connection. The flow now handles both new and existing installations without reinstalling: an existing numeric installation ID is signed into the tenant/project state but remains untrusted until the same GitHub user-access and App checks pass. Public repositories can be verified by canonical GitHub URL through any tenant-bound active App installation, including cross-owner public evidence, while private repositories remain restricted to the installation selection. Workspace exposes the exact-commit import action. Neon Preview branch `br-hidden-mode-ax99b5h6` was created schema-only, an exact Prisma diff reported no drift, all nine existing migrations were baselined, and `prisma migrate status` reported the schema current. The development Clerk organization, user, and membership were reconciled into Preview and a second dry run reported zero drift. Commit `48a3fdb` deployed as Preview `dpl_9wDqskeVdUNzkRkghVrKgi29MTgS` with Preview-only GitHub credentials, and GitHub Actions run `33334829583` passed all hosted gates. The user completed authenticated sign-in and Owner-authorized project creation in the isolated Workspace. Exact GitHub callback/setup URLs are configured. Live installation/import proof, repository-owner recovery or deliberate transfer, runner implementation, artifact storage, and PR reporting remain. |
...
Deploy the commit with the isolated Preview database and Preview-only GitHub
App environment, then configure the exact GitHub App setup and callback URLs.
Create one Preview project, finish the signed GitHub setup callback, connect the
public PlaywrightGen repository by canonical URL, and import its exact-commit
evidence through the existing tenant boundary. Keep remote execution disabled
until the isolated runner gates are implemented.
