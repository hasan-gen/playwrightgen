...
tenant-safe repository import foundation and signed GitHub installation
lifecycle are in progress. Live setup, controlled runner isolation, production
...
| Repository import | Foundation + signed lifecycle in progress | Composite tenant schema, immutable source inventory, read-only GitHub client, Workspace evidence view, raw-body HMAC verification, delivery idempotency, and fail-closed access revocation exist. Add the organization-scoped setup callback, live selected-repository proof, authenticated browser E2E, and Preview evidence. |
